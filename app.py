...
def load_files_from_local(
        keys: list,
        base_path: str = 'local_images_folder'
) -> list:
    """Retrieves files from local directories"""
    local_files = []
    for key in keys:
        try:
            local_file_path = os.path.join(base_path, key)
            local_file_image = Image.open(local_file_path)
            local_files.append(local_file_image)
            # st.write(f"Successfully loaded {key}")
        except Exception as e:
            st.error(f"Error loading {key} from local path: {e}")
    return local_files


@st.cache_data()
def load_local_file_structure(path: str = 'src/all_image_files.json') -> dict:
    """Retrieves JSON document outlining the local file structure"""
    with open(path, 'r') as f:
        return json.load(f)
...
if __name__ == '__main__':
    ...
    all_image_files = load_local_file_structure()
    types_of_birds = sorted(list(all_image_files['test'].keys()))
    types_of_birds = [bird.title() for bird in types_of_birds]

    st.title('AutoVision Birds Species!')
    instructions = """Upload your own image or choose one from the sidebar to use a preconfigured option. The 
    selected or uploaded image will be processed through the Deep Neural Network in real-time, and the output will be 
    ...
    if file:  # if user uploaded file
        ...
        files_to_get_from_local = []

        for im_name in examples_of_species:
            path = os.path.join('train', top_prediction.upper(), im_name)
            files_to_get_from_local.append(path)
        images_from_local = load_files_from_local(keys=files_to_get_from_local)
        ...
        files_to_get_from_local = [key_path]
        examples_of_species = np.random.choice(available_images, size=3)

        for im in examples_of_species:
            path = os.path.join(local_key_prefix, selected_species.upper(), im)
            files_to_get_from_local.append(path)
        images_from_local = load_files_from_local(keys=files_to_get_from_local)
        if images_from_local:  # Check if the list is not empty
            img = images_from_local.pop(0)
            prediction = predict(img, index_to_class_label_dict, model, 5)
        else:
            st.error("No images retrieved from local path. Please check your local configuration and keys.")
            img = None
